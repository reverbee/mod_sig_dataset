...
%   Gray-coded ordering.
%
%   See also FSKMOD, MODEM.PSKMOD, MODEM.PSKDEMOD, MODEM/MODULATE, 
%   MODEM/DEMODULATE.

%   Copyright 1996-2006 The MathWorks, Inc. 
%   $Revision: 1.1.6.5 $  $Date: 2006/05/31 02:46:25 $ 


% Error checks -----------------------------------------------------------------
if (nargin < 4)
    error('comm:fskdemod:numarg', 'Too few input arguments.');
end
...
if (~isreal(M) || ~isscalar(M) || M<=0 || (ceil(M)~=M) || ~isnumeric(M))
    error('comm:fskdemod:Mreal', 'M must be a scalar positive integer.');
end
...
if(nargin==4 || nargin==5 )    
   Symbol_Ordering = 'bin'; %default
    ...
    if (~ischar(Symbol_Ordering)) || (~strcmpi(Symbol_Ordering,'GRAY')) && (~strcmpi(Symbol_Ordering,'BIN'))
        error('comm:fskdemod:SymbolOrder','Invalid symbol set ordering.');    
    end
end
...
if (strcmpi(Symbol_Ordering,'GRAY'))
    [z_degray,gray_map] = gray2bin(z,'fsk',M);   % Gray decode
