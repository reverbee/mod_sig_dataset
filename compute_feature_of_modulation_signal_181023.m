function [feature] = ...
    compute_feature_of_modulation_signal_181023(iqn, amplitude_threshold, feature_name_cell, fs)
% compute feature of modulation signal for modulation classification
%
% ##### 181023 version (bug fixed):
% (1) complex mean computation
% (2) feature(C61, C62, C63, C64) computation
%
% [input]
...
for n = 1 : instance_length
    ...
    feature(n, 10) = feature_C20(iq);
    feature(n, 11) = feature_C21(iq);
    feature(n, 12) = feature_C40(iq);
    feature(n, 13) = feature_C41(iq);
    feature(n, 14) = feature_C42(iq);
    feature(n, 15) = feature_C60(iq);
    feature(n, 16) = feature_C61(iq);
    feature(n, 17) = feature_C62(iq);
    feature(n, 18) = feature_C63(iq);
end
...
% "abs" make C20 real value, not complex
% see [ref (5)] last paragraph in page 11 ~ page 12
C20 = abs(C20);
...
M61 = sum(iq.^5 .* iq(:)') / sample_length;
% M61 = sum(iq.^6 .* iq(:)') / sample_length;
M21 = sum(iq .* iq(:)') / sample_length;
% M21 = sum(iq.^2 .* iq(:)') / sample_length;
M40 = sum(iq.^4) / sample_length;
M20 = sum(iq.^2) / sample_length;
M41 = sum(iq.^3 .* iq(:)') / sample_length;
