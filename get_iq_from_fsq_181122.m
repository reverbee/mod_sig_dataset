function [iq, sample_rate_mhz] = ...
    get_iq_from_fsq_181122(center_freq_mhz, signal_bw_mhz, sample_length, sample_per_symbol, directory, ...
    plot_signal, input_atten_db)
% get iq from R&S FSQ26 signal analyzer
% ################################################################################################################
% [difference from "get_iq_from_fsq_181107.m"]
%
% check advice from r&s online support engineer(zhang, laura)
%
% it needs to turn "free run" off, so please add the following codes before 'TRAC:IQ:DATA?' 
% 
% "INIT:CONT OFF"
% 
...
% [usage]
% (fm broadcasting)
% get_iq_from_fsq_181122(98.5, .192, 2^21, 0, 'e:\fsq_iq\data', 0, '');
% get_iq_from_fsq_181122(98.5, .192, 2^21, 0, 'e:\fsq_iq\data', 0, 10);
% 
