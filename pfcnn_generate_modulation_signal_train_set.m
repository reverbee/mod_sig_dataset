function [] = ...
    pfcnn_generate_modulation_signal_train_set(signal_dir_name, instance_length, ...
    mat_filename_append_string, sample_per_symbol, use_fs, normalize_freq)
% ##### PFCNN model #####
%
% generate modulation signal for cnn model train and save into mat file
%
% modified from "generate_modulation_signal_cnn_train_set - copy(5).m" (190412)
% ######################################################
% differ from "- copy(5).m":
% (1) tdmb: train signal is generated from inf snr signal file
%     computer generated signal file = 'inf_snr_tdmb_fs2.048_fr2.mat'
%     using "tdmb_ofdm_signal.m" 
% #########################################################
%
...
% - use_fs: use fs when compute freq. recommend = 0 (?)
% - normalize_freq: normalize freq. recommend = 0
%
% [usage]
% pfcnn_generate_modulation_signal_train_set('e:\temp\mod_signal', 1000, '_tdmb_fsq_fs6.144', 8, 0, 0)
% pfcnn_generate_modulation_signal_train_set('e:\temp\mod_signal', 1000, '_tdmb_gen_fs4.096', 8, 0, 0)
% pfcnn_generate_modulation_signal_train_set('e:\temp\mod_signal', 1000, '_tdmb', 8, 0, 0)
% pfcnn_generate_modulation_signal_train_set('e:\temp\mod_signal', 1000, '_multi_talk_simpe_radio', 8, 0, 0)
% pfcnn_generate_modulation_signal_train_set('e:\temp\mod_signal', 1000, '_wbfm_fmbroadcasting_fsq_iq', 8, 0, 0)
...
use_tdmb_from_fsq_iq_file = 1;

if use_tdmb_from_fsq_iq_file
    % ##### load tdmb signal from fsq iq file
    % ##### to have fsq iq file, use "get_iq_from_fsq.py"
    ...
    %                   ('sample_rate_mhz', sample_rate_mhz), ('sample_length', iq_length),
    %                   ('timestamp', timestamp)]))
    fsq_iq_filename = 'E:\iq_from_fsq\fs6.144_tdmb_fp1.536\fsq_iq_190409152651_199.280000_6.144000_fp1.536000.mat';

    tdmb = load(fsq_iq_filename);
    % row vector (python numpy default: row major order)
    inf_snr_tdmb_iq = tdmb.iq;
    tdmb_channel_fs_hz = tdmb.sample_rate_mhz * 1e6;
else
    % #### load tdmb signal from file
    % #### to have 'inf_snr_tdmb_fs2.048_fr2.mat',
    % #### use 'tdmb_ofdm_signal.m'
    
    % #### reminding what file have
    % save(signal_filename, 'iq', 'fs', 'frame_length');
    tdmb = load('inf_snr_tdmb_fs4.096_fr2.mat'); % tdmb signal
    % tdmb = load('inf_snr_tdmb_fs2.048_fr2.mat'); % tdmb signal
    inf_snr_tmb_iq = tdmb.iq;
    tdmb_channel_fs_hz = tdmb.fs; % 2.048e6
end
...
modulation_name_cell = {'amsc','ssb','nbfm','wbfm','bpsk','qpsk','2fsk','4fsk','16qam','8vsb','tdmb'};
...
channel_fs_hz_vec = ...
    [44.1e3, 44.1e3, nbfm_channel_fs_hz, wbfm_channel_fs_hz, 1e6, 1e6, 1e6, 1e6, 1e6, ...
    atsc_8vsb_channel_fs_hz, tdmb_channel_fs_hz];
...
% length must be same as modulation_name length
max_freq_offset_hz_vec = [100, 100, 100, 100, 100, 100, 100, 100, 100, 100, 100];
...
%                     max_phase_offset_deg = 0;
            ...
            case 'tdmb'
                if use_tdmb_from_fsq_iq_file
                    ...
                    %                     max_freq_offset_hz = 0;
                    %                     max_phase_offset_deg = 0;
                    iq = inf_snr_gen_tdmb_iq(inf_snr_tdmb_iq, instance_length, iq_sample_length, ...
                        snr_db, [], 0, 0, 0);
                else
                    % ##########################################################
                    % simulate terrestrial dmb signal (daejeon, 199.280, 201.008, 202.736 mhz)
                    % ##########################################################
                    % #### "inf_snr_tmb_iq" loaded from file
                    iq = inf_snr_gen_tdmb_iq(inf_snr_tmb_iq, instance_length, iq_sample_length, snr_db, ...
                        channel_type, channel_fs_hz, max_freq_offset_hz, max_phase_offset_deg);
                end
            otherwise
        ...
        phase_freq = pfcnn_compute_phase_freq(iq, channel_fs_hz, use_fs, normalize_freq);
...
if isempty(mat_filename_append_string)
    mat_filename = sprintf('%s\\PFCNN_%dinst_%dsnr_%dmod.mat', ...
        signal_dir_name, instance_length, snr_length, mod_length);
else
    mat_filename = sprintf('%s\\PFCNN_%dinst_%dsnr_%dmod%s.mat', ...
        signal_dir_name, instance_length, snr_length, mod_length, mat_filename_append_string);
end
...
% ##############################################
param_mat_filename = sprintf('%s\\PFCNN_param.mat', signal_dir_name);
