function [iq, sample_rate_mhz] = ...
    triggered_get_iq_from_fsq_181205(center_freq_mhz, signal_bw_mhz, sample_length, sample_per_symbol, directory, ...
    plot_signal, input_atten_db)
% triggered get iq from R&S FSQ26 signal analyzer
% #############################################################################################
% difference from "triggered_get_iq_from_fsq_181127.m":
% 
% 
% 
% #############################################################################################
...
% [usage]
% (fm broadcasting)
% triggered_get_iq_from_fsq_181205(98.5, .192, 2^17, 0, 'e:\fsq_iq\data', 0, 0);
% triggered_get_iq_from_fsq_181205(98.5, .192, 2^17, 0, 'e:\fsq_iq\data', 0, '');
% 
...
fprintf(fsq_obj, 'TRAC:IQ:DATA:FORM COMP');
...
if count ~= 2
    fprintf(fsq_obj, '*RST');
    fprintf(fsq_obj, 'TRAC:IQ:STAT OFF');
    response = query(fsq_obj, 'TRAC:IQ:STAT?');
    fprintf('trace iq stat = %s\n', response(1:end-1));
