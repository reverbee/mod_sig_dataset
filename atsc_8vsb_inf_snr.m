function [] = atsc_8vsb_inf_snr(symbol_length, sample_per_symbol, signal_plot, signal_save)
% ############### incomplete: (190401) nyquist filter version
...
%
%
% ########### nyquist filter version: 
% ########### for raised cosine fileter version, see "atsc_8vsb_inf_snr_rcos.m"
%
% more accurate code may be necessary: (190327)
% study "trellis coded modulation"
% refer to "commatsc.slx" simulink model: type "commatsc" in matlab command window
%
% [question] 
% ######### "trellis coding implementation in 8vsb atsc signal generation affect classification accuracy?"
%
% ####### how to generate training signal
% (1) load iq from saved inf snr signal file
% (2) select random 128 samples from loaded iq
% (3) apply fading, snr, carrier offset
% (4) symbol synch error is NOT needed because 128 samples are random selected
% 
% [input]
% - symbol_length: symbol length
%   when symbol_length = 2^18, sample_per_symbol = 4, 
%   sample_length = 1048576
%   fix(1048576 / 128) = 8192 instance (greater than 1000 instance)
% - sample_per_symbol: sample per symbol, one of [1,2,3,4,5,6,7]
%   for old(not used) comments on this input, see "atsc_8vsb_inf_snr - copy.m"
% - signal_plot: boolean
% - signal_save: boolean
%
% [usage]
% atsc_8vsb_inf_snr(2^18, 4, 0, 1)
% atsc_8vsb_inf_snr(2^12, 4, 1, 0)
%
...
% replicate symbol as sample per symbol
y = repmat(y, sample_per_symbol, 1);
y = y(:);
size(y)

% if signal_plot
%     plot_signal(y, atsc_symbol_rate, 'after pam mod');
...
% if signal_plot
%     plot_signal_time_domain(y, fs, 'after pam mod');
% end
...
% design nyquist filter using "firnyquist"
n = 40; % filter order
l = 2; % Lth band, 2 = half band
r = .1152; % rolloff factor, see "atsc spec"
b = firnyquist(n, l, r);
...
if signal_plot
    plot_signal(y, fs, 'after nyquist filter');
end
...
% shape = 'normal'; % root raised cosine filter
