function [iq, sample_rate_mhz] = ...
    get_iq_from_fsq_181107(center_freq_mhz, signal_bw_mhz, sample_length, sample_per_symbol, directory, ...
    plot_signal, input_atten_db)
% get iq from R&S FSQ26 signal analyzer
% ################################################################################################################
% [difference from "get_iq_from_fsq.m"]
%
% (1) 'INIT;*WAI' command is removed:
%
%     manual reads 'TRAC:IQ:DATA?' command starts measurement with the settings defined via 'TRACe:IQ:SET' 
%     and returns the list of measurement results immediately,
%     so use of both 'INIT;*WAI' and 'TRAC:IQ:DATA?' is not necessary, may be errorneous
%
% (2) status and error checking routine is removed:
%
%     some error bit(not critical) is set and query register return with empty, why?
%     i give up to fix this problem.
%     but received binary iq data format is exactly same as manual,
%     audio sounding of fm broadcasting signal is very good.
%
% (3) program input("input_atten_db") is added to control rf input attenuation
% ################################################################################################################
%
...
%
% [output]
% - iq: iq data received from fsq
% - sample_rate_mhz: sample rate in mhz set in fsq
%
% [usage]
% (fm broadcasting)
% get_iq_from_fsq_181107(98.5, .192, 2^21, 0, 'e:\fsq_iq\data', 0, '');
% get_iq_from_fsq_181107(98.5, .192, 2^21, 0, 'e:\fsq_iq\data', 0, 10);
% 
...
% query input mixer level
response = query(fsq_obj, 'INP:MIX?');
fprintf('input mixer level = %s dbm\n', response(1:end-1));

% input_atten = sscanf(response(1 : end - 1), '%d');
% fprintf('input attenuation => %d\n', input_atten);
...
% set trigger source to automatic triggering
fprintf(fsq_obj, 'TRIG:SOUR IMM');
...
response = query(fsq_obj, 'SWE:TIME?');
...
% ### below no response: trace iq subsystem is completely different system from spectrum analyzer?
% response = query(fsq_obj, 'FREQ:SPAN?')
% response = query(fsq_obj, 'SWE:TIME?')
