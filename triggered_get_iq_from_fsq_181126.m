function [iq, sample_rate_mhz] = ...
    triggered_get_iq_from_fsq_181126(center_freq_mhz, signal_bw_mhz, sample_length, sample_per_symbol, directory, ...
    plot_signal, input_atten_db)
% triggered get iq from R&S FSQ26 signal analyzer
% #############################################################################################
% difference from "triggered_get_iq_from_fsq_181122.m":
% 
% (1) when fread to read start('#') indicator returned with 0 "count", 
%     send abort command, 'ABOR', to fsq, and prematurely exit
%
% (2) fprintf(fsq_obj, 'TRAC:IQ:DATA?'); => fprintf(fsq_obj, 'TRAC:IQ:DATA?;*WAI');
% 
% #############################################################################################
%
...
% [usage]
% (fm broadcasting)
% triggered_get_iq_from_fsq_181126(95.7, .192, 2^17, 0, 'e:\fsq_iq\data', 0, 0);
% triggered_get_iq_from_fsq_181126(95.7, .192, 2^21, 0, 'e:\fsq_iq\data', 0, '');
% 
